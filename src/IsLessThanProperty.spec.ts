...
import { IsLessThanProperty } from './IsLessThanProperty';

describe('IsLessThanProperty', () => {
  class TestObjectWithoutOpts {
    @IsLessThanProperty('fieldB')
    fieldA: any;
    fieldB: any;
  }
  ...
  class TestObjectWithTransformA {
    @IsLessThanProperty('fieldB', {transformA: (v) => v + 1})
    fieldA: any;
    fieldB: any;
  }
  class TestObjectWithTransformB {
    @IsLessThanProperty('fieldB', {transformB: (v: number) => v - 1})
    fieldA: any;
    fieldB: any;
  }
  class TestObjectWithTransformAB {
    @IsLessThanProperty('fieldB', {transformA: (v) => v + 1, transformB: (v: number) => v - 1})
    fieldA: any;
    fieldB: any;
  }
  ...
  it('should validate without options provided', done => {
    const tests = [
      ...
      {a: 2, b: 1, expect: false }, // 2 < 1
      {a: 0, b: 1, expect: true },  // 0 < 1
    ];
