...
import { IsGreaterThanProperty } from './IsGreaterThanProperty';

describe('IsGreaterThanProperty', () => {
  class TestObjectWithoutOpts {
    @IsGreaterThanProperty('fieldB')
    fieldA: any;
    fieldB: any;
  }
  ...
  class TestObjectWithTransformA {
    @IsGreaterThanProperty('fieldB', {transformA: (v) => v + 1})
    fieldA: any;
    fieldB: any;
  }
  class TestObjectWithTransformB {
    @IsGreaterThanProperty('fieldB', {transformB: (v: number) => v - 1})
    fieldA: any;
    fieldB: any;
  }
  class TestObjectWithTransformAB {
    @IsGreaterThanProperty('fieldB', {transformA: (v) => v + 1, transformB: (v: number) => v - 1})
    fieldA: any;
    fieldB: any;
  }
  ...
  it('should validate without options provided', done => {
    const tests = [
      {a: 1, b: 1, expect: false }, // 1 > 1
      {a: 2, b: 1, expect: true },  // 2 > 1
      {a: 0, b: 1, expect: false }, // 0 > 1
    ];
  ...
  it('should validate with tranformA provided', done => {
    const tests = [
      {a: 1, b: 2, expect: false }, // 1 + 1 > 2
      {a: 1, b: 1, expect: true },  // 1 + 1 > 1
      {a: 0, b: 1, expect: false }, // 0 + 1 > 1
    ];
    const promises = tests.map(({a, b}) => {
  ...
  it('should validate with tranformB provided', done => {
    const tests = [
      {a: 1, b: 3, expect: false },  // 1 > 3 - 1
      {a: 1, b: 2, expect: false },  // 1 > 2 - 1
      {a: 1, b: 1, expect: true  },  // 1 > 1 - 1
    ];
    const promises = tests.map(({a, b}) => {
  ...
  it('should validate with transformA and tranformB provided', done => {
    const tests = [
      ...
      {a:  0, b: 1, expect: true },  //  0 + 1 > 1 - 1
      {a: -1, b: 1, expect: false }, // -1 + 1 > 1 - 1
      {a: -2, b: 1, expect: false }, // -2 + 1 > 1 - 1
    ];
    const promises = tests.map(({a, b}) => {
